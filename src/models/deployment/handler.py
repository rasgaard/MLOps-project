...
class TransformersClassifierHandler(BaseHandler, ABC):
    ...
    def preprocess(self, data):
        ...
        logger.info("Received data: '%s'", sentences)
    ...
    def inference(self, inputs):
        ...
        prediction = self.model(
            **inputs.to(self.device)).logits.softmax(dim=-1).argmax().item()
...
def handle(data, context):
    logger.debug(f"Model dir={context.system_properties.get('model_dir')}")
    try:
